...
import { emptyInputSchema, secureControlOutputSchema } from '../../tool-schemas.js';
export const initiateCardBlockTool = createTool({ id: 'initiate-card-block', description: 'Propose a card block and display server-owned selection. Does not confirm or execute.', inputSchema: emptyInputSchema, outputSchema: secureControlOutputSchema, execute: async (_params, { requestContext }: any) => {
        ...
        return { displayed: true as const, message: 'Customer must use the secure selection and authorization controls.' };
    } });
