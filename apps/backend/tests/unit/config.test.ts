...
describe('Fail-closed configuration',()=>{
 ...
 it('accepts the configured OpenRouter model with its own key',async()=>{vi.stubEnv('APPROVED_AI_PROVIDERS','openrouter');vi.stubEnv('OPENROUTER_MODEL','openrouter/anthropic/claude-sonnet-4');vi.stubEnv('OPENROUTER_API_KEY','test-key');vi.stubEnv('AI_ENABLED','true');const app=await createServer();await app.close();});
